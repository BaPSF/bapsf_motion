# This program translates ASCII commands and sends messages to an
# Applied Motion motor.  It can be used in Motor_Control_3D.py and
# Motor_Control_2D.py for multi-dimensional movement.
...
class Motor_Control:
    ...
    def __init__(self, server_ip_addr=None, msipa_cache_fn=None, verbose=True):
        ...
        # if we get an ip address argument, set that as the suggested
        # server IP address, otherwise look in cache file
        if server_ip_addr is not None:
            self.server_ip_addr = server_ip_addr
        else:
            try:
                # later: save the successfully determined motor server
                #        IP address in a file on disk
                # now: read the previously saved file as a first guess
                #      for the motor server IP address:
    ...
    # Todo : encoder resolution for x/y and z motor is different, but cannot be
    #        changed through command ER
    #
    ...
    def send_text(self, text, timeout: int = None) -> str:
        ...
        if retry_count >= RETRIES:
            input(
                " pausing in motor_control.py send_text() function, hit Enter "
                "to try again, or ^C: "
            )
            s.close()
    ...
    def check_status(self):
        # print("""
        #   # A = An Alarm code is present (use AL command to see code,
        #         AR command to clear code)
    ...
    def inhibit(self, inh=True):
        """
        inh = True:  Raises the disable line on the PWM controller to
                     disable the output
    ...
    def enable(self, en=True):
        """
        en = True:  Lowers the inhibit line on the PWM controller to
                    disable the output
